...
PHOTO = {
 'estate':   IMG + 'WhatsApp_Image_2026-07-21_at_10.21.05_PM__1_.jpeg',
 'finished': IMG + 'WhatsApp_Image_2026-07-21_at_10.21.07_PM.jpeg',
 'tearoff':  IMG + 'WhatsApp_Image_2026-07-21_at_10.21.05_PM.jpeg',
 'aerial':   IMG + 'WhatsApp_Image_2026-07-21_at_10.21.06_PM.jpeg',
 'tile':     IMG + 'WhatsApp_Image_2026-07-21_at_10.21.02_PM.jpeg',
}
ALT = {
...
def fr(key, ratio_cls='', alt=None, sizes='100vw', eager=False, zoom=True, src=None, alt_txt=None):
    u = src or PHOTO[key]
    a = alt_txt if alt_txt is not None else (alt or ALT.get(key, ''))
    cls = 'fr' + (' fr--z' if zoom else '') + ((' ' + ratio_cls) if ratio_cls else '')
    ld  = 'eager" fetchpriority="high' if eager else 'lazy'
    return ('<figure class="%s"><img src="%s" alt="%s" loading="%s" decoding="async" sizes="%s"></figure>'
            % (cls, u, a, ld, sizes))
...
def biz_ld():
    return {"@type":"RoofingContractor","@id":SITE+"/#biz","name":BIZ['name'],
      "url":SITE+"/","telephone":"+1-310-633-5777","image":PHOTO['estate'],
...
<section class="hero">
  <div class="hero__media">
    <img src="{PHOTO['estate']}" alt="{ALT['estate']}" fetchpriority="high" decoding="async">
  </div>
  <div class="hero__in">
